...
CTEST(set_semantics_test, simple_int)
{
    ...
    ASSERT_FALSE(set->contains_from_value(set, &a));
    ASSERT_FALSE(set->contains_from_value(set, &b));
    ASSERT_FALSE(set->contains_from_value(set, &c));
    ASSERT_TRUE(set->add(set, &a));
    ASSERT_TRUE(set->contains_from_value(set, &a));
    ASSERT_TRUE(set->contains_from_value(set, &a));
    ASSERT_TRUE(set->contains_from_value(set, &a));
    ASSERT_TRUE(set->item_count == 1);
    ASSERT_TRUE(set->add(set, &b));
    ASSERT_TRUE(set->contains_from_value(set, &b));
    ASSERT_TRUE(set->contains_from_value(set, &a));
    ASSERT_TRUE(set->item_count == 2);
    ASSERT_TRUE(set->add(set, &c));
    ASSERT_TRUE(set->contains_from_value(set, &a));
    ASSERT_TRUE(set->contains_from_value(set, &b));
    ASSERT_TRUE(set->contains_from_value(set, &c));
...
CTEST(set_semantics_test, remove_string_value)
{
    ...
    ASSERT_TRUE(set->contains_from_value(set, &c));
    ASSERT_TRUE(set->contains_from_value(set, &d));
